...
DMG="$DIST/mirror-x-codex-${VERSION}-macos-${ARCH}.dmg"
ICON_SOURCE="$ROOT/apps/codex-plus-manager/src-tauri/icons/icon.png"
ICON_NAME="mirror-x-codex.icns"
...
prepare_icon() {
  local iconset="$DIST/mirror-x-codex.iconset"
  rm -rf "$iconset"
  mkdir -p "$iconset"
...
<dict>
  ...
  <key>LSUIElement</key>
  <$lsui_element/>
$(if [ "$url_schemes" = "true" ]; then cat <<'URL_TYPES'
...
sign_app() {
  ...
  find "$app_dir/Contents/MacOS" -type f -perm -111 -print0 | while IFS= read -r -d '' nested_executable; do
...
create_app "mirror x codex 管理器" "mirror-x-codex-manager" "$BINARY_DIR/mirror-x-codex-manager" "club.jingziai.mirrorplus.manager" "false" "true"
cp "$BINARY_DIR/mirror-x-imagegen" "$STAGE/mirror x codex 管理器.app/Contents/MacOS/mirror-x-imagegen"
chmod +x "$STAGE/mirror x codex 管理器.app/Contents/MacOS/mirror-x-imagegen"

sign_app "$STAGE/mirror x codex.app"
sign_app "$STAGE/mirror x codex 管理器.app"

verify_app "$STAGE/mirror x codex.app"
verify_app "$STAGE/mirror x codex 管理器.app"

ln -s /Applications "$STAGE/Applications"

for attempt in 1 2 3; do
  if hdiutil create -volname "mirror x codex" -srcfolder "$STAGE" -ov -format UDZO "$DMG"; then
    break
  fi
