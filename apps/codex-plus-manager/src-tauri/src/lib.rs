...
#[cfg(test)]
pub(crate) fn settings_path_test_lock() -> &'static std::sync::Mutex<()> {
...
pub fn run() {
    ...
    let run_result = tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            mobile_control::resume_on_startup();
            Ok(())
        })
        .invoke_handler(tauri::generate_handler![
            ...
            commands::get_mirror_access_status,
            commands::get_mirror_preflight,
            commands::get_windows_sandbox_diagnostic,
            commands::enable_windows_sandbox_access,
            commands::get_mirror_mixed_auth_status,
            codex_setup::install_codex_desktop,
            codex_setup::update_codex_desktop,
            codex_setup::open_codex_store,
            codex_setup::set_codex_app_path,
            commands::validate_mirror_key,
            commands::get_mirror_imagegen_status,
            commands::validate_mirror_image_key,
            commands::enable_mirror_access,
            commands::repair_mirror_sessions,
            commands::preview_session_index_cleanup,
            commands::apply_session_index_cleanup,
            commands::restore_pre_mirror_state,
            commands::recover_mirror_from_baseline,
            commands::load_ads,
            ...
            commands::get_mobile_control_status,
            commands::enable_mobile_control,
            commands::disable_mobile_control,
            commands::generate_mobile_qr_code,
            commands::set_mobile_control_relay_url,
            manager_exit_app,
    ...
    if let Err(error) = run_result {
        ...
        codex_plus_core::windows_show_error_dialog(
            "Mirror X Codex 无法启动",
            &format!(
                "管理器窗口初始化失败。请确认系统已安装 Microsoft Edge WebView2 Runtime，并重新打开。\n\n错误：{error}"
            ),
        );
    }
}
...
/// Restores and focuses the existing Manager window when a second shortcut is opened.
pub fn focus_existing_manager_window() {
    #[cfg(windows)]
    {
        ...
        for process in codex_plus_core::windows_enumerate_processes() {
            ...
            if process
                .exe_file
                .eq_ignore_ascii_case(&format!("{}.exe", codex_plus_core::install::MANAGER_BINARY))
            {
