...
type StepwiseProtocol = "chat_completions" | "responses" | "anthropic_messages" | "auto";
...
function providerSyncProgressMessage(result: CommandResult<ProviderSyncPayload>): string {
  if (result.syncStatus && result.syncStatus !== "synced") {
    return result.syncMessage || result.message || t("历史会话修复已跳过。");
  }
...
export function App() {
  ...
  const refreshPendingProviderImport = async (silent = true) => {
    ...
    if (result) {
      ...
      if (!silent && !isSuccessStatus(result.status)) showResultNotice(t("Mirror X Codex 导入"), result, { silentSuccess: true });
    }
    return result;
  };
  ...
  const confirmPendingProviderImport = async () => {
    ...
    if (result) {
      ...
      setSettings(result);
      setSettingsForm(normalizeSettings(result.settings));
      showResultNotice(t("Mirror X Codex 导入"), result);
      await refreshCcsProviders(true);
    }
  };
  ...
  const dismissPendingProviderImport = async () => {
    ...
    if (result) {
      setPendingProviderImport(null);
      showResultNotice(t("Mirror X Codex 导入"), result, { silentSuccess: true });
    }
  };
  ...
  const restart = async () => {
    ...
    if (result) {
      showNotice(t("重启 Mirror X Codex"), result.message, result.status);
      await refreshOverview(true);
    }
  };
  ...
  useEffect(() => {
    if (getLanguage() === "en") {
      void invoke("update_tray_labels", {
        ...
        quitLabel: "Quit",
        windowTitle: "Mirror X Codex 管理器",
      });
    }
  }, []);
  ...
  return (
    <div className={`shell ${theme}`}>
      <aside className="sidebar">
        <div className="brand">
          <div className="brand-mark">镜</div>
          <div className="brand-copy">
            <div className="brand-title-row">
              <div className="brand-title">Mirror X Codex</div>
              {hasUpdate ? (
                <button
        ...
        <header className="topbar" key={`topbar-${route}`}>
          ...
          <div className="topbar-actions">
            ...
            >
              ...
              <Button onClick={() => void actions.restart()} title={t("重启 Mirror X Codex")} variant="outline">
...
function OverviewScreen({
  ...
  return (
    <>
      <Panel className="provider-overview">
        <CardContent>
          <div className="provider-overview-layout">
            <div className="provider-overview-main">
              <div className="provider-overview-mark">
                <Network className="h-5 w-5" />
              </div>
              <div>
                <span className="eyebrow">{t("推荐接入")}</span>
                <h2>Mirror X Codex</h2>
                <p>
                  {t("已为 Mirror X Codex 预配置好接入参数。新建供应商时直接选择 Mirror X Codex 模板，只需填写 API Key 即可在 Codex 中使用本站可用模型。")}
                </p>
              </div>
            </div>
            <div className="provider-overview-side">
              <div className="provider-overview-tags">
                <span>gpt-5.4</span>
                <span>gpt-5.5</span>
                <span>responses</span>
              </div>
              <Button onClick={() => void actions.openExternalUrl("https://api.jingziai.club/pricing")}>
...
function EnhanceScreen({
  ...
  const remoteMarketplaceSummary = remotePluginMarketplace?.marketplaceRoot
    ...
    : t("未发现本地缓存；点击按钮会从 Mirror X Codex 内置快照释放并注册，无需官方账号预缓存。");
  return (
    <>
      <Panel>
        ...
        <CardContent>
          ...
          <div className="enhance-feature-groups">
            <FeatureGroup title={t("插件与模型")} detail={t("管理插件市场、模型列表和服务档位相关增强。")}>
              ...
              <div className="feature-action-row">
                <div>
                  ...
                  <small>{t("使用 Mirror X Codex 内置快照补齐远端插件，API 模式也可显示和安装 Product Design 插件。")}</small>
            ...
            <FeatureGroup title={t("界面与启动")} detail={t("控制语言、启动速度和 Codex 原生界面调整。")}>
              ...
              <FeatureToggle title={t("原生菜单栏位置")} detail={t("把 Mirror X Codex 菜单插入 Codex 顶部原生菜单栏。")} checked={form.codexAppNativeMenuPlacement} disabled={!masterEnabled} onChange={(value) => setEnhanceFlag("codexAppNativeMenuPlacement", value)} />
            ...
            <FeatureGroup title={t("远程项目")} detail={t("连接 Zed Remote 和 upstream worktree 辅助能力。")}>
              ...
              <FeatureToggle title={t("Zed 项目记录")} detail={t("维护 Mirror X Codex 自己的远程项目最近列表。")} checked={form.zedRemoteProjectRegistryEnabled} disabled={!masterEnabled} onChange={(value) => setEnhanceFlag("zedRemoteProjectRegistryEnabled", value)} />
...
function ZedRemoteScreen({
  ...
  return (
    <>
      <Panel>
        <CardHead title={t("Zed 远程项目")} detail={tf("{0} 个 Mirror X Codex 可识别项目，默认策略：{1}", [allProjects.length, zedStrategyLabel(form.zedRemoteOpenStrategy)])} />
        <CardContent>
          ...
          <div className="zed-remote-settings">
            ...
            <label className="switch-row compact">
              ...
              <span>
                ...
                <small>{t("保存到 Mirror X Codex state，不改写 Zed settings。")}</small>
              </span>
            </label>
          </div>
          <Toolbar>
  ...
  return (
    <>
      <Panel>
        ...
        <CardContent>
          <div className="metric-list">
            <Metric label={t("本页会话")} value={tf("{0} 个", [items.length])} />
            <Metric label={t("本页未归档")} value={tf("{0} 个", [activeCount])} />
            <Metric label={t("本页已归档")} value={tf("{0} 个", [archivedCount])} />
...
function RecommendationsScreen({ ads, actions }: { ads: AdsResult | null; actions: Actions }) {
  ...
  return (
    <>
      <Panel>
        <CardHead title={t("推荐内容")} detail={t("默认推荐内容已禁用")} />
        <CardContent>
          <div className="recommend-hero">
            <div>
              <strong>{t("当前未提供推荐内容")}</strong>
              <span>{t("已移除默认远端广告与赞助内容。")}</span>
            </div>
...
function MaintenanceScreen({
  ...
  return (
    <>
      ...
      <Panel>
        ...
        <CardContent>
          <label className="check-row">
            ...
            <span>{t("卸载时移除 Mirror X Codex 托管数据")}</span>
          </label>
          <Toolbar>
      ...
      <Panel>
          <CardHead title={t("自动接管")} detail={t("Watcher 用于保持 Mirror X Codex 接管状态")} />
        <CardContent>
          <Toolbar>
      ...
      <Panel>
        ...
        <CardContent>
          ...
          <Toolbar>
        <Button onClick={() => void actions.launch()}>{t("启动 Mirror X Codex")}</Button>
...
function AboutScreen({
  ...
  return (
    <>
      <Panel>
        <CardHead title={t("关于 Mirror X Codex")} detail={t("本地 Codex 增强、管理工具和安装包维护")} />
        <CardContent>
          <div className="metric-list">
          <Metric label={t("Mirror X Codex 版本")} value={overview?.current_version ?? update?.currentVersion ?? "-"} />
            <Metric label={t("Codex 版本")} value={overview?.codex_version ?? t("未检测到")} />
            <Metric label={t("服务地址")} value="api.jingziai.club/pricing" />
          </div>
          <Toolbar>
            <Button onClick={() => void actions.openExternalUrl("https://api.jingziai.club/pricing")} variant="secondary">
              <ExternalLink className="h-4 w-4" />
              {t("打开服务页面")}
            </Button>
          </Toolbar>
            ...
            <Field label={t("上游协议")}>
              <select
                ...
                onChange={(event) =>
                  onFormChange({
                    ...form,
                    codexAppStepwiseProtocol: event.currentTarget.value as StepwiseProtocol,
                  })
                }
              >
                <option value="auto">{t("自动兼容")}</option>
    ...
    <div className="relay-profile-editor">
      ...
      {showApiFields && profile.protocol === "chatCompletions" ? (
        <div className="hint-line relay-protocol-hint">
          ...
          <span>{t("此上游会通过本地 127.0.0.1:57321 转成 Responses API，需要从 Mirror X Codex 启动 Codex。")}</span>
        </div>
      ) : null}
      <div className="hint-line relay-protocol-hint">
        <ShieldCheck className="h-4 w-4" />
  ...
  return (
    <div className="modal-backdrop" role="dialog" aria-modal="true">
      <div className="modal-card provider-import-modal">
        <div className="modal-head">
          <div>
            <h2>{t("导入 Mirror X Codex 供应商")}</h2>
            <p>{t("检测到来自网页的供应商配置导入请求，确认后会写入本机 Mirror X Codex 管理器。")}</p>
          </div>
...
function routeSubtitle(route: Route) {
  const subtitles: Record<Route, string> = {
    ...
    recommendations: t("默认已禁用的推荐内容"),
    maintenance: t("入口安装、修复、Watcher 与手动启动"),
    about: t("版本信息、GitHub Release 更新、日志与诊断"),
    settings: t("主题和启动参数"),
  };
...
function healthItems(overview: OverviewResult | null) {
  return [
    ...
    },
    {
      ...
      detail: overview?.silent_shortcut.path || t("缺少 Mirror X Codex 静默启动快捷方式时可在安装维护页修复。"),
    },
    {
      title: t("管理工具入口"),
...
function relayProfileReadinessText(profile: RelayProfile, relay: RelayResult | null): string {
  if (relay?.stateUnreadable) {
    return relay.stateError || t("Codex 配置状态不可读，请先恢复 config.toml / auth.json 后再切换供应商。");
  }
  if (isAggregateRelayProfile(profile)) {
    const aggregate = normalizeAggregateConfig(profile.aggregate, []);
...
function applyRelayProfilePatchToFiles(
  ...
  if ("model" in patch) {
    // 模型后缀（如 [1M]）仅供 mirrorplus 内部使用，写入 config.toml 前需剥离，
