...
async fn main() -> Result<()> {
    ...
    let Some(_guard) = acquire_single_instance_guard(options.debug_port)? else {
        ...
        focus_existing_codex_without_launch(&options, &process_ids)?;
        return Ok(());
    };
    let process_ids = codex_plus_core::watcher::find_codex_processes();
...
fn report_launch_failure(error: &anyhow::Error) {
    let manager_result = open_manager_after_launch_failure();
    let _ = codex_plus_core::diagnostic_log::append_diagnostic_log(
        "launcher.launch_failed",
        json!({
            "error": format!("{error:#}"),
            "manager_opened": manager_result.is_ok(),
            "manager_error": manager_result.as_ref().err().map(ToString::to_string),
        }),
    );
    #[cfg(windows)]
    {
        let guidance = if manager_result.is_ok() {
            "已打开管理器，请按页面提示安装、选择或修复 Codex。"
        } else {
            "管理器也无法打开，请重新安装完整的 Mirror X Codex 安装包。"
        };
        codex_plus_core::windows_show_error_dialog(
            "Mirror X Codex 启动失败",
            &format!("{guidance}\n\n错误：{error:#}"),
        );
    }
...
fn open_manager_after_launch_failure() -> anyhow::Result<()> {
    ...
    match try_acquire_single_instance_guard() {
        ...
        Err(error) if error.kind() == std::io::ErrorKind::AddrInUse => {
            Err(error).with_context(|| {
                format!(
                    "launcher guard port {} is occupied by another process; no launcher or Codex process was terminated",
...
fn focus_existing_codex_without_launch(
    ...
    let (launch_status, launch_message) = existing_codex_status(process_ids, activated);
    let _ = options
        .status_store
        .save_latest(&codex_plus_core::status::LaunchStatus {
            status: launch_status.to_string(),
            message: launch_message.to_string(),
    ...
    let _ = codex_plus_core::diagnostic_log::append_diagnostic_log(
        "launcher.existing_codex_left_untouched",
        json!({
            "debug_port": options.debug_port,
            "helper_port": options.helper_port,
            "process_ids": process_ids,
            "activated": activated,
            "codex_state_writes": false,
            "codex_launch_attempted": false,
        }),
    );
    Ok(())
}

fn existing_codex_status(process_ids: &[u32], activated: bool) -> (&'static str, &'static str) {
    if process_ids.is_empty() {
        return (
            "failed",
            "The previous Mirror X Codex launcher is still cleaning up; no Codex process was detected and no second start was attempted.",
        );
    }
    if activated {
        return (
            "running_degraded",
            "Existing Codex window activated without changing its files; mirror enhancements were not re-injected or verified.",
        );
    }
    (
        "failed",
        "A Codex background process was detected, but no window could be activated. No second instance was started.",
    )
}
...
impl LaunchHooks for LauncherHooks {
    ...
    async fn ensure_managed_provider_ready(
        ...
        self.core.ensure_managed_provider_ready(settings).await
    }
...
async fn try_inject_with_context(
    ...
    let settings = codex_plus_core::settings::SettingsStore::default()
        ...
        .map_err(|error| {
            anyhow::anyhow!(
                "读取桌面增强设置失败：{error:#}；已停止重注入，未使用默认设置覆盖当前功能"
            )
        })?;
...
fn default_user_scripts_config_dir() -> PathBuf {
    if let Some(path) = preferred_user_scripts_config_dir() {
        return path;
    }
    default_primary_user_scripts_config_dir()
}

fn preferred_user_scripts_config_dir() -> Option<PathBuf> {
    let current = default_primary_user_scripts_config_dir();
    let legacy = legacy_user_scripts_config_dir();
    if user_scripts_config_present(&current) {
        return Some(current);
    }
    if user_scripts_config_present(&legacy) {
        return Some(legacy);
    }
    None
}

fn user_scripts_config_present(path: &Path) -> bool {
    path.join("user_scripts.json").exists() || path.join("user_scripts").exists()
}

fn default_primary_user_scripts_config_dir() -> PathBuf {
    if cfg!(windows) {
        if let Some(roaming) = std::env::var_os("APPDATA") {
            return PathBuf::from(roaming).join("mirrorplus");
        }
        if let Some(home) = directories::BaseDirs::new().map(|dirs| dirs.home_dir().to_path_buf()) {
            return home.join("AppData").join("Roaming").join("mirrorplus");
        }
    }
...
mod tests {
    ...
    fn launcher_uses_single_instance_guard_before_launching() {
        ...
        let guard = source
            .split_once("fn acquire_single_instance_guard")
            .unwrap()
            .1
            .split_once("fn try_acquire_single_instance_guard")
            .unwrap()
            .0;
        assert!(!guard.contains("stop_launcher_processes"));
        assert!(!guard.contains("should_recover_stale_launcher"));
        assert!(guard.contains("no launcher or Codex process was terminated"));
    }

    #[test]
    fn existing_codex_path_never_launches_or_injects_again() {
        let source = include_str!("main.rs");
        let existing = source
            .split_once("fn focus_existing_codex_without_launch")
            .unwrap()
            .1
            .split_once("fn log_launcher_already_running")
            .unwrap()
            .0;

        assert!(!existing.contains(".launch_codex("));
        assert!(!existing.contains("start_helper"));
        assert!(!existing.contains("ensure_injection"));
        assert!(existing.contains("codex_launch_attempted\": false"));
        assert!(existing.contains("codex_state_writes\": false"));
    }

    #[test]
    fn existing_codex_never_reports_enhancements_ready_without_verification() {
        let (activated_status, activated_message) = existing_codex_status(&[42], true);
        let (manual_status, manual_message) = existing_codex_status(&[42], false);
        let (guard_status, _) = existing_codex_status(&[], false);

        assert_eq!(activated_status, "running_degraded");
        assert!(activated_message.contains("not re-injected or verified"));
        assert_eq!(manual_status, "failed");
        assert!(manual_message.contains("no window could be activated"));
        assert_eq!(guard_status, "failed");
    }

    #[test]
    fn user_scripts_config_prefers_legacy_when_current_missing() {
        let base =
            std::env::temp_dir().join(format!("mirrorplus-user-scripts-{}", std::process::id()));
        let current = base.join("mirrorplus");
        let legacy = base.join("Codex++");
        std::fs::create_dir_all(legacy.join("user_scripts")).unwrap();

        assert!(!user_scripts_config_present(&current));
        assert!(user_scripts_config_present(&legacy));
    }

    #[test]
    fn user_scripts_config_detects_current_when_present() {
        let base = std::env::temp_dir().join(format!(
            "mirrorplus-user-scripts-current-{}",
            std::process::id()
        ));
        let current = base.join("mirrorplus");
        std::fs::create_dir_all(&current).unwrap();
        std::fs::write(current.join("user_scripts.json"), "{}").unwrap();
    ...
    async fn watchdog_reuses_bridge_context_with_data_service() {
        let test_dir = std::env::temp_dir().join(format!(
            "mirrorplus-launcher-watchdog-test-{}",
            std::process::id()
        ));
        ...
        let result = codex_plus_core::routes::handle_bridge_request(
            hooks.watchdog_bridge_context().unwrap(),
    ...
    #[test]
    fn launch_failure_opens_manager_and_surfaces_a_windows_error() {
        let source = include_str!("main.rs");

        assert!(source.contains("report_launch_failure(&error)"));
        assert!(source.contains("open_manager_after_launch_failure"));
        assert!(source.contains("windows_show_error_dialog"));
        assert!(source.contains("manager_opened"));
    }
}
