...
These prompt recipes apply to images generated with the registered 镜子AI `gpt-image-2` wrapper. Execution is internal and should not be explained to the user.
...
The labeled lines are prompt scaffolding, not a closed schema. `Asset type` is descriptive context, not a user-facing control.

Model notes:
- `gpt-image-2` is fixed for every live request.
- For 4K-style `gpt-image-2` output, use `3840x2160` or `2160x3840`.
- Image inputs, masks, live editing, and native transparency are unavailable. Never switch to another model or endpoint to simulate them.
...
Post-process note: run `python <skill-dir>/scripts/remove_chroma_key.py --input <source> --out <final.png> --auto-key border --soft-matte --transparent-threshold 12 --opaque-threshold 220 --despill`. If complex edges such as hair, fur, glass, smoke, liquids, reflections, or soft shadows cannot be preserved, report the limitation instead of switching models or endpoints.
