#!/usr/bin/env python3
"""Codex-style CLI for image generation through the 镜子AI gateway.

This adapted copy preserves the official imagegen command shape and structured
prompt workflow while routing generation to the site's gpt-image-2 model.
"""
...
def _load_gateway_config() -> Dict[str, str]:
    ...
    try:
        ...
        _die(f"Unable to read gateway config {path}: {exc}")
...
def _resolve_api_key() -> str:
    ...
    return (
        os.getenv("JINGZI_API_KEY")
        or os.getenv("MIRROR_API_KEY")
...
def _ensure_api_key(dry_run: bool) -> None:
    if _resolve_api_key():
        print("JINGZI_API_KEY is configured.", file=sys.stderr)
        return
    if dry_run:
        _warn("JINGZI_API_KEY is not configured; dry-run only.")
        return
    _die(
        "JINGZI_API_KEY is not configured. Set it locally or run "
...
def _validate_model(model: str) -> None:
    if model != GPT_IMAGE_2_MODEL:
        _die("This 镜子AI skill currently supports only gpt-image-2.")
...
def _validate_model_specific_options(
    *,
    ...
    if background == "transparent":
        _die(
            "Native transparent backgrounds are not supported by the 镜子AI "
            "gpt-image-2 gateway. Generate a flat chroma-key background and use "
            "scripts/remove_chroma_key.py instead."
        )
...
def _create_async_client():
    ...
    return AsyncOpenAI(api_key=_resolve_api_key(), base_url=_resolve_base_url())


def _image_item_to_b64(item: Any) -> str:
    ...
    if not image_url:
        _die("Image response contains neither b64_json nor url.")

    request = Request(
        str(image_url),
        headers={
            "Accept": "image/*",
            "User-Agent": "jingzi-imagegen-skill/1.0",
        },
    )
    try:
        ...
        _die(f"Temporary image download failed with HTTP {exc.code}.")
    except URLError as exc:
        _die(f"Temporary image download failed: {exc.reason}")

    if not content_type.lower().startswith("image/"):
        _die(f"Temporary image link returned non-image content: {content_type or 'unknown'}")
    if not data:
        _die("Temporary image link returned an empty file.")
...
async def _run_generate_batch(args: argparse.Namespace) -> int:
    ...
    async def run_job(i: int, job: Dict[str, Any]) -> Tuple[int, Optional[str]]:
        ...
        try:
            ...
            images = [_image_item_to_b64(item) for item in result.data]
...
def _edit(args: argparse.Namespace) -> None:
    if not args.dry_run:
        _die(
            "Image editing is not supported by the current 镜子AI gpt-image-2 "
            "gateway. Use generate or generate-batch."
        )
    prompt = _read_prompt(args.prompt, args.prompt_file)
    prompt = _augment_prompt(args, prompt)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Codex-style image generation through the 镜子AI gpt-image-2 gateway"
    )
