...
impl std::fmt::Display for SelectionError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            SelectionError::ModelNotRouted {
                aggregate_id,
                model,
            } => write!(
                formatter,
                "聚合供应商「{aggregate_id}」没有为模型「{model}」配置 Key"
            ),
        }
    }
}
...
pub fn select_relay_for_request(
    ...
    if let Some(relays) =
        model_routed_relays(settings, &active_aggregate, context.model.as_deref())?
    {
...
pub fn fallback_relays_for_request(
    settings: &BackendSettings,
    relay_id: &str,
    model: Option<&str>,
) -> Result<Vec<RelayProfile>, SelectionError> {
    ...
    if let Some(relays) = model_routed_relays(settings, &active_aggregate, model)? {
        let start_index = relays
            .iter()
            .position(|relay| relay.id == relay_id)
            .map(|index| index + 1)
            .unwrap_or(0);
        return Ok((0..relays.len().saturating_sub(1))
            .map(|offset| relays[(start_index + offset) % relays.len()].clone())
            .collect());
    }
    fallback_relays_after(settings, relay_id)
}
...
fn model_routed_relays(
    settings: &BackendSettings,
    aggregate: &AggregateRelayProfile,
    model: Option<&str>,
) -> Result<Option<Vec<RelayProfile>>, SelectionError> {
    let Some(model) = model.map(str::trim).filter(|model| !model.is_empty()) else {
        return Ok(None);
    };
    let relays = aggregate
        .members
        .iter()
        .map(|member| {
            relay_profile_by_id(settings, &member.relay_id).ok_or_else(|| {
    ...
    if !relays
        .iter()
        .any(|relay| !relay.model_list.trim().is_empty())
    {
        return Ok(None);
    }
    let eligible = relays
        .into_iter()
        .filter(|relay| {
            relay.model_list.trim().is_empty()
                || relay
                    .model_list
                    .split(['\r', '\n', ','])
                    .map(str::trim)
    ...
    if eligible.is_empty() {
        return Err(SelectionError::ModelNotRouted {
            aggregate_id: aggregate.id.clone(),
