...
/// Asynchronous callback used by the watchdog to restore a launcher-specific data bridge.
...
pub trait LaunchHooks: Send + Sync {
    ...
    async fn validate_codex_state_before_writes(
        &self,
        _settings: &BackendSettings,
        app_dir: &Path,
    ) -> anyhow::Result<()> {
        validate_codex_state_before_writes_for_app(Some(app_dir))?;
        #[cfg(windows)]
        crate::codex_app_state::set_local_full_access_mode(
            &crate::codex_home::resolve_codex_home().path,
        )
        .context("Codex 启动前同步 Full access 失败；未创建 Codex 进程")?;
        Ok(())
    }
    async fn ensure_managed_provider_ready(
        &self,
        _settings: &BackendSettings,
    ) -> anyhow::Result<()> {
        Ok(())
    ...
    H: IntoLaunchHooks,
{
    ...
    let settings = match hooks.load_settings().await {
        ...
        Err(error) => {
            let message = format!(
                "无法读取 Mirror X Codex 设置：{error:#}；为避免使用默认值启动或改写配置，已停止启动"
            );
            ...
            let _ = options.status_store.save_latest(&failure);
    ...
    let result: anyhow::Result<LaunchHandle> = async {
        hooks
            .validate_codex_state_before_writes(&settings, &app_dir)
            .await?;
        hooks
            .ensure_managed_provider_ready(&settings)
            .await
            .with_context(|| "Codex 启动前 Mirror 托管供应商校验或修复失败；未创建 Codex 进程")?;
        ...
        // Plugin initialization can update config.toml. Re-check the narrowly
        // owned Mirror provider fields after that writer has finished.
        hooks
            .ensure_managed_provider_ready(&settings)
            .await
            .with_context(
                || "Codex 启动前最终 Mirror 托管供应商校验或修复失败；未创建 Codex 进程",
            )?;
        // Historical session repair is an explicit maintenance action. A normal launch must not
        // rewrite Codex databases, even when an older model suffix is detected.
            ...
            .map_err(|error| {
                if protocol_proxy_enabled && error_is_address_in_use(&error) {
                    ...
                    return error.context(format!(
                        "协议代理端口 {helper_port} 被其他进程占用，等待 {} 秒后仍未释放。请退出占用该端口的程序后重试。",
        ...
        if settings.enhancements_enabled {
            ...
            if injection_ready {
                ...
                let degraded = launch_status(
                    "running_degraded",
                    "Codex launched; mirror+ enhancements are still waiting for the page bridge.",
                    debug_port,
                    helper_port,
                    &app_dir,
                );
...
pub fn validate_codex_state_before_writes()
-> anyhow::Result<crate::project_config::ProjectConfigScan> {
    validate_codex_state_before_writes_for_app(None)
}

fn validate_codex_state_before_writes_for_app(
    app_dir: Option<&Path>,
) -> anyhow::Result<crate::project_config::ProjectConfigScan> {
    let process_ids = crate::watcher::find_codex_processes();
    if let Some(message) = running_codex_block_message(&process_ids) {
        anyhow::bail!(message);
    }
    crate::codex_home::validate_codex_home_environment()
        .with_context(|| "Codex 启动前 CODEX_HOME 检查失败")?;
    crate::codex_sqlite::validate_codex_sqlite_home_environment()
        .with_context(|| "Codex 启动前 CODEX_SQLITE_HOME 检查失败")?;

    let home = crate::codex_home::resolve_codex_home().path;
    let sqlite_home = crate::codex_sqlite::resolve_codex_sqlite_home().path;
    let state_dir = crate::paths::default_app_state_dir();
    for path in crate::mirror_access::codex_runtime_storage_paths(
        &home,
        &state_dir,
        ...
        crate::mirror_access::ensure_storage_headroom(
            &path,
            0,
            crate::mirror_access::MIN_CODEX_RUNTIME_FREE_SPACE_BYTES,
        )?;
    }

    let expected_provider = crate::project_config::active_provider_from_home(&home)?;
    let scan = crate::project_config::scan_recent_project_configs(&home, &expected_provider, 64);
    let _ = crate::diagnostic_log::append_diagnostic_log(
        "launcher.project_config_scan",
        serde_json::json!({
            "codex_home": home,
    ...
    if let Some(message) = active_provider_override_message(&scan) {
        anyhow::bail!(message);
    }
    Ok(scan)
}

fn running_codex_block_message(process_ids: &[u32]) -> Option<String> {
    (!process_ids.is_empty()).then(|| {
        format!(
            "检测到 Codex 已在运行（{} 个进程）。为避免改写运行中的配置、插件或会话索引，本次已在任何 Codex 状态写入前停止；请完全退出 Codex 后再启动 Mirror X Codex。",
            process_ids.len()
        )
    })
}

fn active_provider_override_message(
    scan: &crate::project_config::ProjectConfigScan,
) -> Option<String> {
    let provider_overrides = scan
        .overrides
        .iter()
    ...
    let examples = provider_overrides
        .iter()
        .take(3)
        .map(|entry| format!("{} ({})", entry.config_path, entry.detail))
        .collect::<Vec<_>>()
        .join("；");
    Some(format!(
        "检测到 {} 个近期项目配置会改变 active provider，已在任何 Codex 状态写入前停止启动：{}",
        provider_overrides.len(),
        examples
    ))
}
...
#[cfg(windows)]
fn apply_mirrorplus_window_icon_after_launch(process_id: u32) {
    let icon_resource_path = std::env::current_exe()
        .unwrap_or_else(|_| PathBuf::from(format!("{}.exe", crate::install::SILENT_BINARY)));
    tokio::spawn(async move {
        for attempt in 1..=30 {
            if crate::windows_apply_mirrorplus_icon_to_process_window(
                process_id,
...
#[cfg(not(windows))]
fn apply_mirrorplus_window_icon_after_launch(_process_id: u32) {}
...
impl LaunchHooks for DefaultLaunchHooks {
    ...
    async fn ensure_managed_provider_ready(
        ...
        crate::mirror_access::ensure_managed_provider_ready(&home, &state_dir, &settings_path)?;
        Ok(())
    }
    ...
    async fn ensure_plugin_marketplace_config(
        ...
        let home = crate::relay_config::default_codex_home_dir();
        let (remote, curated, role_specific) = ensure_required_plugin_marketplaces(&home)?;
        let _ = crate::diagnostic_log::append_diagnostic_log(
            "launcher.plugin_marketplaces_ready",
            serde_json::json!({
                "home": home,
                "remote_initialized": remote.initialized,
                "remote_configured": remote.configured,
                "curated_configured": curated,
                "role_specific_configured": role_specific,
            }),
        );
        Ok(())
    }
    ...
    async fn launch_codex(
        ...
        if cfg!(windows) {
            ...
            if let Some(activation) = activation {
                ...
                apply_mirrorplus_window_icon_after_launch(process_id);
    ...
    async fn wait_for_codex_exit(
        ...
        wait_for_existing_codex_exit(debug_port).await
    }
...
fn ensure_required_plugin_marketplaces(
    home: &Path,
) -> anyhow::Result<(
    crate::plugin_marketplace::MarketplaceEnsureResult,
    bool,
    bool,
)> {
    let remote =
        crate::plugin_marketplace::ensure_openai_curated_remote_marketplace_available(home)
            .with_context(|| "Codex 启动前准备内置插件市场失败")?;
    if let Err(error) =
        crate::plugin_marketplace::commit_openai_curated_remote_marketplace_initialization(home)
    {
        let _ = crate::diagnostic_log::append_diagnostic_log(
            "launcher.plugin_marketplace_backup_cleanup_failed",
            serde_json::json!({
                "error": error.to_string(),
            }),
        );
    }
    let curated = crate::plugin_marketplace::ensure_openai_curated_marketplace_config(home)
        .with_context(|| "Codex 启动前保留本地插件市场配置失败")?;
    let role_specific =
        crate::plugin_marketplace::ensure_role_specific_plugins_marketplace_config(home)
            .with_context(|| "Codex 启动前保留角色插件配置失败")?;
    if crate::plugin_marketplace::openai_curated_remote_marketplace_status(home).needs_repair() {
        anyhow::bail!("内置插件市场写入后校验失败，已停止启动 Codex");
    }
    Ok((remote, curated, role_specific))
}
        ...
        Err(error) => {
            return (
                ...
                serde_json::to_vec(&serde_json::json!({
                    "status": "failed",
                    "message": format!(
                        "读取图片覆盖层设置失败：{error:#}；未使用默认设置继续读取图片"
                    )
                }))
                .unwrap_or_default(),
                "application/json; charset=utf-8".to_string(),
                "helper.overlay_image_settings_failed",
            );
        }
    };
...
async fn handle_protocol_proxy_connection(
    ...
    let upstream = match crate::protocol_proxy::open_responses_proxy_request_at_path(
        request_body,
        request_user_agent,
        path,
    )
...
/// Keep a launcher alive while an already-open Codex instance remains reachable.
///
/// This is used by the single-instance activation path, where there is no
/// child process handle for `wait_for_codex_exit` to own.
pub async fn wait_for_existing_codex_exit(debug_port: u16) -> anyhow::Result<()> {
    let mut empty_streak = 0u32;
    loop {
        let has_codex_process = !crate::watcher::find_codex_processes().is_empty();
        // macOS cannot enumerate Codex processes through the Windows watcher,
        // so CDP remains the portable liveness signal for an existing app.
...
async fn try_inject(debug_port: u16, helper_port: u16) -> anyhow::Result<()> {
    ...
    let websocket_url = target
        ...
        .as_deref()
        .ok_or_else(|| anyhow::anyhow!("selected CDP target has no websocket URL"))?;
    let settings = SettingsStore::default().load().map_err(|error| {
        anyhow::anyhow!("读取桌面增强设置失败：{error:#}；已停止注入，未使用默认设置覆盖当前功能")
    })?;
...
mod tests {
    ...
    fn address_in_use_error() -> anyhow::Error {
        std::io::Error::new(std::io::ErrorKind::AddrInUse, "test port busy").into()
    }

    #[tokio::test]
    async fn fixed_helper_port_recovers_after_a_short_bind_conflict() {
        let mut attempts = 0;
        start_helper_waiting_for_busy_port(
            || {
                attempts += 1;
    ...
    #[tokio::test]
    async fn helper_bind_does_not_retry_unrelated_failures() {
        let mut attempts = 0;
        let error = start_helper_waiting_for_busy_port(
            || {
                attempts += 1;
    ...
    #[test]
    fn project_model_override_is_diagnostic_only() {
        let scan = crate::project_config::ProjectConfigScan {
            scanned_projects: 1,
            overrides: vec![crate::project_config::ProjectConfigOverride {
                project_path: "project".to_string(),
                config_path: "project/.codex/config.toml".to_string(),
                model_provider: None,
                model: Some("gpt-5".to_string()),
                profile: None,
                changes_active_provider: false,
                detail: "项目固定模型 gpt-5".to_string(),
            }],
            unreadable_configs: Vec::new(),
        };

        assert!(active_provider_override_message(&scan).is_none());
    }

    #[test]
    fn ignored_project_provider_override_does_not_block_launch() {
        let scan = crate::project_config::ProjectConfigScan {
            scanned_projects: 1,
            overrides: vec![crate::project_config::ProjectConfigOverride {
                project_path: "project".to_string(),
                config_path: "project/.codex/config.toml".to_string(),
                model_provider: Some("official".to_string()),
                model: None,
                profile: None,
                changes_active_provider: false,
                detail: "项目写入 Provider official（Codex 项目配置层会忽略此键）".to_string(),
            }],
            unreadable_configs: Vec::new(),
        };
    ...
    #[test]
    fn running_codex_blocks_all_prelaunch_state_writes() {
        let message = running_codex_block_message(&[41, 42]).unwrap();

        assert!(message.contains("2 个进程"));
        assert!(message.contains("任何 Codex 状态写入前停止"));
        assert!(message.contains("插件"));
        assert!(running_codex_block_message(&[]).is_none());
    }

    #[test]
    fn required_plugin_marketplaces_initialize_and_verify_empty_home() {
        let temp = tempfile::tempdir().unwrap();

        let (remote, curated, role_specific) =
            ensure_required_plugin_marketplaces(temp.path()).unwrap();

        assert!(remote.initialized);
        assert!(remote.configured);
        assert!(!curated);
        assert!(!role_specific);
        assert!(
            !crate::plugin_marketplace::openai_curated_remote_marketplace_status(temp.path())
                .needs_repair()
        );
    }

    #[test]
    fn required_plugin_marketplaces_preserve_damaged_config_bytes() {
        let temp = tempfile::tempdir().unwrap();
        let config_path = temp.path().join("config.toml");
        let original = b"model = [\r\n";
        std::fs::write(&config_path, original).unwrap();

        let error = ensure_required_plugin_marketplaces(temp.path()).unwrap_err();

        assert!(
            error
                .to_string()
                .contains("Codex 启动前准备内置插件市场失败")
        );
    ...
    #[test]
    fn chunked_decoder_handles_binary_body_and_trailers() {
    ...
    #[test]
    fn content_length_body_enforces_limit_and_completeness() {
        assert_eq!(content_length_body(b"abc", 3).unwrap(), b"abc");
        assert!(content_length_body(b"ab", 3).is_err());
        assert_eq!(
