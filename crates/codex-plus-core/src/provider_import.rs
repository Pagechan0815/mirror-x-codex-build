...
pub fn import_provider_with_store(
    ...
    let mut settings = store.load().map_err(|error| {
        anyhow::anyhow!("读取现有供应商设置失败：{error:#}；已停止导入，未改写 settings.json")
    })?;
...
fn build_config_toml(base_url: &str, api_key: &str, protocol: RelayProtocol) -> String {
    ...
    [
        "model_provider = \"mirrorplus\"".to_string(),
        String::new(),
        "[model_providers.mirrorplus]".to_string(),
        "name = \"mirrorplus\"".to_string(),
...
mod tests {
    ...
    #[test]
    fn parses_mirrorplus_provider_url() {
        let url = "mirrorplus://v1/import/provider?resource=provider&name=mirror%2B&baseUrl=https%3A%2F%2Fapi.jingziai.club%2Fv1&apiKey=sk-test&wireApi=responses&relayMode=pureApi&configContents=bW9kZWxfcHJvdmlkZXIgPSAibWlycm9ycGx1cyIK&authContents=eyJPUEVOQUlfQVBJX0tFWSI6InNrLXRlc3QifQo%3D";

        let request = request_from_url(url).unwrap();

        assert_eq!(request.name, "mirror+");
        assert_eq!(request.base_url, "https://api.jingziai.club/v1");
    ...
    fn imports_provider_once_and_selects_it() {
        ...
        let request = ProviderImportRequest {
            ...
            base_url: "https://api.jingziai.club/v1/".to_string(),
    ...
    fn pending_provider_import_round_trips_and_clears() {
        ...
        assert_eq!(pending.name, "mirror+");
        assert_eq!(pending.base_url, "https://api.jingziai.club/v1");
    ...
    #[test]
    fn import_rejects_corrupt_settings_without_replacing_them() {
        let dir = tempfile::tempdir().unwrap();
        ...
        let error = import_provider_with_store(request, store)
            .expect_err("corrupt settings must block provider import");

        assert!(error.to_string().contains("读取现有供应商设置失败"));
