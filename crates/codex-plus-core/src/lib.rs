...
#[cfg(windows)]
pub fn windows_show_error_dialog(title: &str, message: &str) {
    windows_integration::show_error_dialog(title, message)
}
...
#[cfg(windows)]
pub fn windows_apply_mirrorplus_icon_to_process_window(
    process_id: u32,
    icon_resource_path: std::path::PathBuf,
) -> bool {
    windows_integration::apply_mirrorplus_icon_to_process_window(process_id, icon_resource_path)
}
