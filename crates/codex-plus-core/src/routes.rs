...
impl BridgeSettingsService for CoreSettingsService {
    ...
    async fn codex_app_version(&self) -> anyhow::Result<String> {
        ...
        let settings = self.store.load().map_err(|error| {
            anyhow::anyhow!("读取 Codex 应用设置失败：{error:#}；未使用默认路径继续探测")
        })?;
...
fn settings_payload_value(
    ...
    let active_relay_codex_provider = settings
        .relay_profiles
        .iter()
        .find(|profile| profile.id == settings.active_relay_id)
        .map(|profile| {
            crate::model_catalog::codex_model_provider_for_relay_profile(codex_home, profile)
        })
        .unwrap_or_default();
    let active_relay_session_provider = if active_relay_codex_provider.is_empty() {
        ""
    } else if active_relay_codex_provider.eq_ignore_ascii_case("openai") {
        "openai"
    } else {
        "custom"
    };
    ...
    if let Some(object) = value.as_object_mut() {
        ...
        object.insert(
            ...
            Value::String(active_relay_session_provider.to_string()),
        );
        object.insert(
...
async fn settings_value(
    ...
    let codex_home = ctx.settings.codex_home();
    settings_payload_value(settings, codex_app_version, &codex_home)
}
