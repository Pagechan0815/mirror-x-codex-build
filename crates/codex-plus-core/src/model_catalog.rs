...
const MODEL_SOURCE_REQUEST_TIMEOUT: Duration = Duration::from_secs(20);
...
pub async fn read_codex_model_catalog() -> Value {
    ...
    let client = match crate::http_client::proxied_client("mirrorplus/1.0") {
        Ok(client) => client,
        Err(error) => {
            return json!({
                "status": "failed",
...
pub fn codex_model_provider_for_relay_profile(home: &Path, profile: &RelayProfile) -> String {
    let profile_provider = profile.codex_provider_id();
    if !profile_provider.is_empty() {
        return profile_provider;
    }

    let (live_config, effective, error) = load_codex_config(&home.join("config.toml"));
    if error.is_some() {
        return String::new();
    }
    let live_provider = string_value(effective.get("model_provider"));
    if live_provider == "openai" || live_config.model_providers.contains_key(&live_provider) {
        live_provider
    } else {
        String::new()
    }
...
async fn fetch_models_from_source(
    ...
    let mut request = client
        ...
        .timeout(MODEL_SOURCE_REQUEST_TIMEOUT);
