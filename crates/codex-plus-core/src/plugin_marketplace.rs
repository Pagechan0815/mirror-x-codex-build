...
const CODEX_PLUS_MARKETPLACE: &str = "mirror-x-curated";
...
pub fn ensure_openai_curated_remote_marketplace_config(home: &Path) -> anyhow::Result<bool> {
    ...
    let mut changed = cleanup_managed_reserved_marketplace_configs(home)?;
    changed |= ensure_marketplace_configs(home, &[CODEX_PLUS_MARKETPLACE], &marketplace_root)?;
    Ok(changed)
}
...
pub fn ensure_openai_curated_remote_marketplace_available(
    ...
    let configured = match ensure_openai_curated_remote_marketplace_config(home) {
        Ok(configured) => configured,
        Err(error) if initialized => {
            return match rollback_openai_curated_remote_marketplace_initialization(home) {
                Ok(()) => Err(error).context(
                    "failed to register embedded plugin marketplace; initialization was rolled back",
                ),
                Err(rollback_error) => Err(error).context(format!(
                    "failed to register embedded plugin marketplace and rollback failed: {rollback_error}"
                )),
            };
        }
...
pub fn rollback_openai_curated_remote_marketplace_initialization(
    home: &Path,
) -> anyhow::Result<()> {
    let plugins_root = home.join(".tmp");
    let destination = plugins_root.join("plugins-remote");
    let backup = plugins_root.join("plugins-remote.previous-codex-plus");
    if !destination.starts_with(&plugins_root) || destination == plugins_root {
        anyhow::bail!("refusing to remove plugin marketplace outside CODEX_HOME/.tmp");
    }
    if !backup.starts_with(&plugins_root) || backup == plugins_root {
        anyhow::bail!("refusing to restore plugin marketplace outside CODEX_HOME/.tmp");
    }
    if destination.exists() {
        std::fs::remove_dir_all(&destination)
            .with_context(|| format!("failed to rollback {}", destination.display()))?;
    }
    if backup.exists() {
...
/// Finalize a successful marketplace replacement only after the surrounding
/// Mirror access transaction can no longer roll back.
pub fn commit_openai_curated_remote_marketplace_initialization(home: &Path) -> anyhow::Result<()> {
    let plugins_root = home.join(".tmp");
    let backup = plugins_root.join("plugins-remote.previous-codex-plus");
    if !backup.starts_with(&plugins_root) || backup == plugins_root {
        anyhow::bail!("refusing to remove plugin marketplace backup outside CODEX_HOME/.tmp");
    }
    match std::fs::remove_dir_all(&backup) {
        Ok(()) => Ok(()),
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => Ok(()),
        Err(error) => Err(error)
            .with_context(|| format!("failed to remove committed backup {}", backup.display())),
    }
}
...
async fn download_openai_plugins_zip() -> anyhow::Result<Vec<u8>> {
    let client =
        crate::http_client::proxied_client(&format!("mirrorplus/{}", crate::version::VERSION))?;
    let bytes = client
...
fn rewrite_marketplace_name(root: &Path) -> anyhow::Result<()> {
    ...
    crate::settings::atomic_write(&path, encoded.as_bytes())
}
...
fn install_openai_curated_remote_marketplace_zip(home: &Path, bytes: &[u8]) -> anyhow::Result<()> {
    ...
    let result = extract_zip_exact(bytes, &staging)
        ...
        .and_then(|_| {
            replace_directory_with_backup_name_and_policy(
                &staging,
                &destination,
...
fn replace_directory(source: &Path, destination: &Path) -> anyhow::Result<()> {
    replace_directory_with_backup_name_and_policy(
        source,
        destination,
        "plugins.previous-codex-plus",
        false,
    )
}

fn replace_directory_with_backup_name_and_policy(
    source: &Path,
    destination: &Path,
    backup_name: &str,
    retain_backup_on_success: bool,
) -> anyhow::Result<()> {
    let backup = destination.with_file_name(backup_name);
    if backup.exists() {
        anyhow::bail!(
            "refusing to overwrite uncommitted marketplace backup {}",
            backup.display()
        );
    }
    if destination.exists() {
    ...
    match std::fs::rename(source, destination) {
        Ok(()) => {
            if backup.exists() && !retain_backup_on_success {
...
pub fn cleanup_managed_reserved_marketplace_configs(home: &Path) -> anyhow::Result<bool> {
    ...
    if let Some(marketplaces) = doc.get_mut("marketplaces").and_then(Item::as_table_mut) {
        for (name, root) in managed_entries {
...
fn marketplace_table_points_to_root(table: &Table, root: &Path) -> bool {
    table.get("source_type").and_then(Item::as_str) == Some("local")
        && table
            ...
            .is_some_and(|source| normalize_windows_extended_path(source) == root.to_string_lossy())
}
...
mod tests {
    ...
    #[test]
    fn ensure_openai_curated_marketplace_config_registers_non_reserved_remote_marketplace() {
        ...
        assert!(
            parsed["marketplaces"]
                .get("openai-curated-remote")
                .is_none()
        );
        assert_eq!(
            parsed["marketplaces"][CODEX_PLUS_MARKETPLACE]["source_type"].as_str(),
            Some("local")
        );
        assert_eq!(
            parsed["marketplaces"][CODEX_PLUS_MARKETPLACE]["source"].as_str(),
            Some(
                format!(
                    r"\\?\{}",
    ...
    #[test]
    fn embedded_remote_marketplace_rollback_restores_replaced_directory() {
        let temp = tempfile::tempdir().unwrap();
        let home = temp.path();
        let root = home.join(".tmp").join("plugins-remote");
        std::fs::create_dir_all(&root).unwrap();
        std::fs::write(root.join("customer-cache.txt"), "keep me").unwrap();
        ...
        assert!(
            home.join(".tmp")
                .join("plugins-remote.previous-codex-plus")
                .join("customer-cache.txt")
                .is_file()
        );
        rollback_openai_curated_remote_marketplace_initialization(home).unwrap();
        assert_eq!(
            std::fs::read_to_string(root.join("customer-cache.txt")).unwrap(),
            "keep me"
        );
        assert!(
    ...
    #[test]
    fn embedded_remote_marketplace_commit_discards_replaced_directory_backup() {
        let temp = tempfile::tempdir().unwrap();
        let home = temp.path();
        let root = home.join(".tmp").join("plugins-remote");
        std::fs::create_dir_all(&root).unwrap();
        std::fs::write(root.join("obsolete-cache.txt"), "old").unwrap();

        ensure_openai_curated_remote_marketplace_available(home).unwrap();
        commit_openai_curated_remote_marketplace_initialization(home).unwrap();
        ...
        assert!(!root.join("obsolete-cache.txt").exists());
    }

    #[test]
    fn embedded_remote_marketplace_rolls_back_when_config_registration_fails() {
        let temp = tempfile::tempdir().unwrap();
        let home = temp.path();
        std::fs::write(home.join("config.toml"), "not = [valid").unwrap();

        let error = ensure_openai_curated_remote_marketplace_available(home).unwrap_err();

        assert!(error.to_string().contains("rolled back"));
        assert!(!home.join(".tmp").join("plugins-remote").exists());
        assert_eq!(
    ...
    #[test]
    fn failed_remote_marketplace_registration_preserves_replaced_directory() {
        let temp = tempfile::tempdir().unwrap();
        let home = temp.path();
        let root = home.join(".tmp").join("plugins-remote");
        std::fs::create_dir_all(&root).unwrap();
        ...
        ensure_openai_curated_remote_marketplace_available(home).unwrap_err();
    ...
    fn install_openai_plugins_zip_installs_valid_snapshot() {
        ...
        assert!(
            !changed,
            "reserved OpenAI marketplace names must not be registered locally"
        );
        assert!(
            temp.path()
