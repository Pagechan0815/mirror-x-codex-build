...
use windows::Win32::UI::WindowsAndMessaging::{
    EnumWindows, GetWindowThreadProcessId, IsIconic, IsWindowVisible, MB_ICONERROR, MB_OK,
    MessageBoxW, PostMessageW, SW_RESTORE, SetForegroundWindow, ShowWindow, WM_CLOSE,
};
#[cfg(windows)]
use windows::Win32::UI::WindowsAndMessaging::{
...
#[cfg(windows)]
pub fn apply_mirrorplus_icon_to_process_window(
    process_id: u32,
...
#[cfg(windows)]
pub fn request_process_close(process_id: u32) -> anyhow::Result<bool> {
    let Some(hwnd) = visible_window_for_process(process_id) else {
        return Ok(false);
    };
    unsafe {
        PostMessageW(hwnd, WM_CLOSE, WPARAM(0), LPARAM(0))
            .with_context(|| format!("无法请求进程 {process_id} 正常关闭"))?;
    }
    Ok(true)
}
...
fn apply_taskbar_properties(hwnd: HWND, icon_resource_path: &PathBuf) -> anyhow::Result<()> {
    ...
    set_property_string(
        ...
        "club.jingziai.mirrorplus.codex",
    )?;
    set_property_string(
        &store,
