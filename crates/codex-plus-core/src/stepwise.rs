...
pub async fn test_connection(settings: &BackendSettings) -> anyhow::Result<Value> {
    generate(
        StepwiseRequest {
            ...
            thread_title: "mirror+ Stepwise test".to_string(),
            page_url: String::new(),
        },
        settings,
    )
...
mod tests {
    ...
    #[test]
    fn extracts_items_from_protocol_text_blocks() {
        for response in [
            json!({
                "choices": [{
                    "message": {
            ...
            json!({
                "output": [{
                    "content": [{
                        "type": "output_text",
                        "text": "{\"items\":[{\"prompt\":\"解析 Responses 文本块\"}]}"
                    }]
                }]
            }),
            json!({
                "content": [{
                    "type": "text",
                    "text": "{\"items\":[{\"prompt\":\"解析 Anthropic 文本块\"}]}"
                }]
            }),
        ] {
            assert_eq!(extract_stepwise_items(&response, 6).len(), 1);
        }
    }
    ...
    #[tokio::test]
    async fn generate_supports_chat_responses_and_anthropic_protocols() {
        let cases = [
            (
                "chat_completions",
                "/chat/completions",
                json!({"choices":[{"message":{"content":"{\"items\":[{\"prompt\":\"chat ok\"}]}"}}]}),
                "chat ok",
            ),
            (
                "responses",
                "/responses",
                json!({"output_text":"{\"items\":[{\"prompt\":\"responses ok\"}]}"}),
                "responses ok",
            ),
            (
                "anthropic_messages",
                "/messages",
                json!({"content":[{"type":"text","text":"{\"items\":[{\"prompt\":\"anthropic ok\"}]}"}]}),
                "anthropic ok",
            ),
        ];

        for (protocol, request_path, response, expected) in cases {
            ...
            let result = generate(test_request(), &test_settings(server.uri(), protocol))
                .await
                .unwrap();

            assert_eq!(result["status"], "ok");
            assert_eq!(result["protocol"], protocol);
            assert_eq!(result["items"][0]["prompt"], expected);
    ...
    #[tokio::test]
    async fn auto_protocol_falls_back_only_for_protocol_mismatch() {
        let server = MockServer::start().await;
        Mock::given(method("POST"))
        ...
        Mock::given(method("POST"))
            ...
            .respond_with(ResponseTemplate::new(200).set_body_json(json!({
                "output_text": "{\"items\":[{\"prompt\":\"fallback ok\"}]}"
            })))
            .mount(&server)
            .await;
        ...
        Mock::given(method("POST"))
            ...
            .mount(&auth_server)
            .await;
        let failed = generate(test_request(), &test_settings(auth_server.uri(), "auto"))
            .await
            .unwrap();
        assert_eq!(failed["status"], "failed");
        assert_eq!(failed["protocol"], "chat_completions");
        assert!(failed["error"].as_str().unwrap().contains("[redacted]"));
        assert!(!failed["error"].as_str().unwrap().contains(TEST_API_KEY));
