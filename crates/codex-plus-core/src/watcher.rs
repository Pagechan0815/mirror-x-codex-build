...
pub const CODEX_GRACEFUL_CLOSE_TIMEOUT_MS: u64 = 15_000;
pub const LAUNCHER_GRACEFUL_EXIT_TIMEOUT_MS: u64 = 30_000;
const RESTART_STOP_WAIT_INTERVAL_MS: u64 = 100;
pub const WATCHER_RUN_NAME: &str = "MirrorPlusWatcher";
pub const WATCHER_RUN_KEY: &str = r"Software\Microsoft\Windows\CurrentVersion\Run";
pub const WATCHER_STARTUP_SHORTCUT_NAME: &str = "MirrorPlusWatcher.lnk";
...
fn is_windowsapps_codex_app_process(executable: &str) -> bool {
    ...
    let supported_package = [
        "\\windowsapps\\openai.codex_",
        "\\windowsapps\\openai.codexbeta_",
        "\\windowsapps\\openai.chatgpt-desktop_",
    ]
    .iter()
    .any(|identity| executable.contains(identity));
    supported_package
        && !executable.contains("\\app\\resources\\")
        && (executable.ends_with("\\app\\codex.exe") || executable.ends_with("\\app\\chatgpt.exe"))
}

#[cfg(windows)]
fn is_standalone_codex_app_process(
    ...
    process.executable_path.as_deref().is_some_and(|path| {
        let path = path
            ...
            .replace('/', "\\")
            .to_ascii_lowercase();
        !path.contains("\\windowsapps\\") && path.ends_with("\\chatgpt.exe")
    })
}
    ...
    processes
        ...
        .filter(|(process_id, _, exe_file)| {
            !protected.contains(process_id)
                && exe_file.eq_ignore_ascii_case(&format!("{}.exe", crate::install::SILENT_BINARY))
        })
...
pub fn find_codex_processes_from_snapshot(
    ...
    // lowercase codex.exe here; that is commonly the CLI binary. Current standalone
    // builds may use ChatGPT.exe under the OpenAI\Codex application directory.
...
#[cfg(windows)]
pub fn stop_launcher_processes_and_wait() -> bool {
    let processes = crate::windows_integration::enumerate_processes();
    let killable = filter_killable_launcher_processes(
        processes.iter().map(|process| {
            (
...
/// Wait for launchers that were already running to finish their own cleanup.
///
/// A launcher can still own provider/session-sync locks after Codex exits. The
/// restart path must never terminate it during that window, otherwise the next
/// launch can observe a partial sync or a stale owner lock.
#[cfg(windows)]
pub fn wait_for_launcher_processes_to_exit() -> bool {
    let processes = crate::windows_integration::enumerate_processes();
    let launcher_ids = filter_killable_launcher_processes(
        processes.iter().map(|process| {
            (
                process.process_id,
                process.parent_process_id,
    ...
    wait_for_processes_to_exit(
        launcher_ids,
        LAUNCHER_GRACEFUL_EXIT_TIMEOUT_MS,
        RESTART_STOP_WAIT_INTERVAL_MS,
        "watcher.launcher_graceful_exit_timeout",
    )
}
...
#[cfg(windows)]
pub fn request_codex_shutdown_and_wait() -> bool {
    let process_ids = find_codex_processes();
    if process_ids.is_empty() {
        return true;
    }
    let requested = process_ids
        .iter()
        .filter_map(|process_id| {
            crate::windows_integration::request_process_close(*process_id)
                .ok()
    ...
    if requested == 0 {
        let _ = crate::diagnostic_log::append_diagnostic_log(
            "watcher.codex_graceful_close_unavailable",
            serde_json::json!({ "process_ids": process_ids }),
        );
        return false;
    }

    let deadline =
        std::time::Instant::now() + Duration::from_millis(CODEX_GRACEFUL_CLOSE_TIMEOUT_MS);
    loop {
        ...
        if std::time::Instant::now() >= deadline {
            let _ = crate::diagnostic_log::append_diagnostic_log(
                "watcher.codex_graceful_close_timeout",
                serde_json::json!({
                    "remaining_process_ids": remaining,
                    "timeout_ms": CODEX_GRACEFUL_CLOSE_TIMEOUT_MS
                }),
            );
            return false;
        }
...
fn create_startup_shortcut(launcher_path: &Path, arguments: &str) -> anyhow::Result<()> {
    ...
    crate::windows_integration::create_shortcut(&crate::windows_integration::ShortcutSpec {
        ...
        description: "mirror+ watcher".to_string(),
        icon: None,
        show_minimized: true,
    })
}
