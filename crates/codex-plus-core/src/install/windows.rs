...
const UNINSTALL_SUBKEY: &str = r"Software\Microsoft\Windows\CurrentVersion\Uninstall\MirrorXCodex";
const LEGACY_UNINSTALL_SUBKEY: &str =
    r"Software\Microsoft\Windows\CurrentVersion\Uninstall\MirrorPlus";
const OLDEST_UNINSTALL_SUBKEY: &str =
    r"Software\Microsoft\Windows\CurrentVersion\Uninstall\Codex++";
const URL_PROTOCOL_SUBKEY: &str = r"Software\Classes\mirrorplus";
...
pub fn build_windows_entrypoint_plan(options: &InstallOptions) -> WindowsEntrypointPlan {
    ...
    WindowsEntrypointPlan {
        silent_shortcut: install_root
            .join("mirror x codex.lnk")
            .to_string_lossy()
            .to_string(),
        manager_shortcut: install_root
            .join("mirror x codex 管理器.lnk")
            .to_string_lossy()
            .to_string(),
        ...
        uninstall_key: "MirrorXCodex".to_string(),
        legacy_uninstall_key: "MirrorPlus".to_string(),
...
pub fn uninstall_shortcuts(options: &InstallOptions) -> anyhow::Result<()> {
    ...
    let _ = crate::windows_integration::delete_current_user_key(OLDEST_UNINSTALL_SUBKEY);
...
fn write_uninstall_registration(plan: &WindowsEntrypointPlan) -> anyhow::Result<()> {
    ...
    let install_location = Path::new(&plan.manager_path)
        ...
        .to_string();
    for (name, value) in [
        ("DisplayName", "mirror x codex".to_string()),
        ("DisplayVersion", crate::version::VERSION.to_string()),
        ("Publisher", "镜子AI".to_string()),
...
#[cfg(windows)]
fn cleanup_legacy_uninstall_registrations() {
    let _ = crate::windows_integration::delete_current_user_key(OLDEST_UNINSTALL_SUBKEY);
    let _ = crate::windows_integration::delete_current_user_key(LEGACY_UNINSTALL_SUBKEY);
}
...
fn register_url_protocol(manager_path: &str) -> anyhow::Result<()> {
    crate::windows_integration::set_current_user_string_value(
        URL_PROTOCOL_SUBKEY,
        "",
        "URL:mirror x codex Import Protocol",
    )?;
    crate::windows_integration::set_current_user_string_value(
        URL_PROTOCOL_SUBKEY,
        "URL Protocol",
...
fn default_icon_path() -> PathBuf {
    std::env::current_exe()
        .ok()
        .unwrap_or_else(|| PathBuf::from(format!("{MANAGER_BINARY}.exe")))
}
