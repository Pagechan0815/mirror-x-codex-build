...
pub async fn check_for_update(current_version: &str) -> anyhow::Result<UpdateCheck> {
    if DEFAULT_LATEST_JSON_URL.trim().is_empty() {
        return Ok(UpdateCheck {
            current_version: current_version.to_string(),
            latest_version: None,
            release_summary: "默认更新源已禁用。".to_string(),
            asset_name: None,
            asset_url: None,
...
fn is_macos_native_arch_asset(name: &str) -> bool {
    ...
    // Legacy filename shape used underscores before platform names.
...
fn is_windows_installer_asset(name: &str) -> bool {
    (name.contains("codex") && (name.contains("plus") || name.contains("mirror-x"))
        || name.contains("mirrorplus"))
        && (name.ends_with(".msi")
