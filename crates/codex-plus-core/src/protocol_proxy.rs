...
pub fn upstream_http_client() -> anyhow::Result<reqwest::Client> {
    reqwest::Client::builder()
        ...
        .user_agent("mirrorplus/ProtocolProxy")
        .build()
...
pub async fn open_responses_proxy_request(
    ...
    open_responses_proxy_request_at_path(body, original_user_agent, "/responses").await
}

pub async fn open_responses_proxy_request_at_path(
    body: &str,
    original_user_agent: Option<&str>,
    request_path: &str,
) -> anyhow::Result<UpstreamProxyResponse> {
    let settings = SettingsStore::default().load().map_err(|error| {
        anyhow::anyhow!(
            "读取供应商设置失败：{error:#}；已停止代理请求，未使用默认 Provider 继续转发"
        )
    })?;
...
async fn open_responses_proxy_request_with_settings_and_user_agent(
    ...
    relays.extend(crate::relay_rotation::fallback_relays_for_request(
        &settings,
        &relay.id,
...
pub async fn open_models_proxy_request(
    ...
    let settings = SettingsStore::default().load().map_err(|error| {
        anyhow::anyhow!(
            "读取供应商设置失败：{error:#}；已停止模型列表请求，未使用默认 Provider 继续转发"
        )
    })?;
...
pub async fn open_audio_transcriptions_proxy_request(
    ...
    let settings = SettingsStore::default().load().map_err(|error| {
        anyhow::anyhow!(
            "读取供应商设置失败：{error:#}；已停止语音转写请求，未使用默认 Provider 继续转发"
        )
    })?;
    let relay = crate::relay_rotation::select_relay_for_probe(&settings)?;
    validate_upstream(&relay)?;
...
pub async fn open_chat_completions_proxy_request(
    ...
    let settings = SettingsStore::default().load().map_err(|error| {
        anyhow::anyhow!(
            "读取供应商设置失败：{error:#}；已停止 Chat Completions 请求，未使用默认 Provider 继续转发"
        )
    })?;
...
fn upstream_request_parts(
    ...
    match relay.protocol {
        RelayProtocol::Responses => {
            ...
            normalize_responses_custom_tool_call_ids(&mut request_json);
            Ok((
                responses_url_for_request_path(&relay.base_url, request_path),
                request_json,
...
pub fn responses_url_for_request_path(base_url: &str, request_path: &str) -> String {
