...
impl RelayProfile {
    pub fn codex_provider_id(&self) -> String {
        if self.relay_mode == RelayMode::Official && !self.official_mix_api_key {
            return "openai".to_string();
        }

        let Some(doc) = self.config_contents.parse::<DocumentMut>().ok() else {
            return String::new();
        };
...
pub struct BackendSettings {
    ...
    #[serde(
        rename = "mobileControlRelayUrl",
        default = "default_mobile_control_relay_url",
        deserialize_with = "empty_as_default_mobile_control_relay_url"
    )]
...
impl BackendSettings {
    ...
    pub fn active_relay_codex_provider(&self) -> String {
        self.relay_profiles
            .iter()
            .find(|profile| profile.id == self.active_relay_id)
            .map(RelayProfile::codex_provider_id)
            .unwrap_or_default()
    }

    pub fn active_relay_session_provider(&self) -> &'static str {
        let provider = self.active_relay_codex_provider();
...
pub fn default_mobile_control_relay_url() -> String {
    "wss://relay.jingziai.club/relay".to_string()
}

pub fn empty_as_default_mobile_control_relay_url<'de, D>(
    deserializer: D,
    ...
    D: serde::Deserializer<'de>,
{
    ...
    Ok(value
        ...
        .filter(|value| !value.is_empty())
        .unwrap_or_else(default_mobile_control_relay_url))
}
...
impl SettingsStore {
    ...
    pub fn load(&self) -> anyhow::Result<BackendSettings> {
        let Some(contents) = read_settings_with_legacy_migration(&self.path)? else {
            return Ok(BackendSettings::default());
        };

        Ok(normalize_settings_config_sections(
            deserialize_settings_contents(&contents, &self.path)?,
        ))
    }

    pub fn save(&self, settings: &BackendSettings) -> anyhow::Result<()> {
        validate_existing_settings_before_write(&self.path)?;
    ...
    pub fn update(&self, payload: Value) -> anyhow::Result<BackendSettings> {
        ...
        let settings = normalize_settings_config_sections(deserialize_settings_object(
            raw.clone(),
            &self.path,
        )?);
        raw.insert(
    ...
    fn load_raw_object(&self) -> anyhow::Result<Map<String, Value>> {
        ...
        let raw = parse_settings_object(&contents, &self.path)?;
        deserialize_settings_object(raw.clone(), &self.path)?;
        Ok(raw)
    }
}

fn parse_settings_object(contents: &str, path: &Path) -> anyhow::Result<Map<String, Value>> {
    let value = serde_json::from_str::<Value>(contents).with_context(|| {
        format!(
            "设置文件 {} 不是有效 JSON；原文件已保留，未使用默认设置覆盖",
            path.display()
        )
    })?;
    let Value::Object(map) = value else {
        anyhow::bail!(
            "设置文件 {} 的根节点必须是 JSON object；原文件已保留，未使用默认设置覆盖",
            path.display()
        );
    };
    Ok(map)
...
fn deserialize_settings_object(
    raw: Map<String, Value>,
    path: &Path,
) -> anyhow::Result<BackendSettings> {
    serde_json::from_value(Value::Object(raw)).with_context(|| {
        format!(
            "设置文件 {} 包含不兼容的字段或字段类型；原文件已保留，未使用默认设置覆盖",
            path.display()
        )
    })
}

fn deserialize_settings_contents(contents: &str, path: &Path) -> anyhow::Result<BackendSettings> {
    deserialize_settings_object(parse_settings_object(contents, path)?, path)
}

fn validate_existing_settings_before_write(path: &Path) -> anyhow::Result<()> {
    match fs::read_to_string(path) {
        Ok(contents) => {
            deserialize_settings_contents(&contents, path)?;
            return Ok(());
        }
    ...
    if path != crate::paths::default_settings_path() {
        return Ok(());
    }
    let legacy = crate::paths::legacy_app_state_dir().join("settings.json");
    match fs::read_to_string(&legacy) {
        Ok(contents) => {
            deserialize_settings_contents(&contents, &legacy)?;
            Ok(())
        }
...
fn read_settings_with_legacy_migration(path: &Path) -> anyhow::Result<Option<String>> {
    match fs::read_to_string(path) {
    ...
    deserialize_settings_contents(&contents, &legacy)
        .context("旧版设置迁移已中止，损坏的原文件未被写入当前设置")?;
    atomic_write(path, contents.as_bytes()).with_context(|| {
        format!(
            "failed to migrate legacy settings {} to {}",
            legacy.display(),
            path.display()
        )
    })?;
...
pub fn atomic_write(path: &Path, bytes: &[u8]) -> anyhow::Result<()> {
    ...
    // 每次使用唯一的同目录临时文件，避免两个后台操作共用固定 `.tmp` 互相覆盖。
    // 写入并 flush 到磁盘后才替换目标，低磁盘或进程中断时不会留下截断配置。
    let file_name = path
        .file_name()
    ...
    let write_result = (|| -> anyhow::Result<()> {
        ...
        // Preserve the existing permission bits where the platform exposes them.
        if let Ok(metadata) = fs::metadata(path) {
            let _ = fs::set_permissions(&temp_path, metadata.permissions());
        }
        replace_temp_path(&temp_path, path).with_context(|| {
            format!(
...
/// Replace an existing file with a same-directory staging file.
///
/// Rust's Windows implementation handles native long paths. A short retry
/// window covers transient antivirus or indexer sharing conflicts. Read-only
/// targets are made replaceable only after their permissions have been copied
/// to the staging file, and are restored if the commit fails.
pub fn replace_temp_path(temp_path: &Path, target: &Path) -> std::io::Result<()> {
    #[cfg(windows)]
    {
...
mod tests {
    ...
    #[cfg(windows)]
    #[test]
    fn atomic_write_replaces_a_readonly_file_without_losing_permissions() {
        let dir = temp_dir();
        let path = dir.join("readonly-settings.json");
    ...
    #[cfg(windows)]
    #[test]
    fn atomic_write_supports_paths_beyond_legacy_max_path() {
        use std::os::windows::ffi::OsStrExt;

        let root = temp_dir();
        let mut dir = root.clone();
        while dir.as_os_str().encode_wide().count() <= 280 {
            dir.push("long-path-segment");
        }
    ...
    #[test]
    fn settings_deserialize_normalizes_stepwise_protocol_and_legacy_default() {
...
experimental_bearer_token = "sk-existing""#
    ...
    fn settings_store_load_bad_json_returns_error_and_preserves_file() {
        ...
        assert!(error.to_string().contains("未使用默认设置覆盖"));
        assert_eq!(std::fs::read(path).unwrap(), original);
    }

    #[test]
    fn settings_store_load_non_object_root_returns_error_and_preserves_file() {
        let dir = temp_dir();
        let path = dir.join("settings.json");
        let original = br#"[{"providerSyncEnabled":true}]"#;
        std::fs::write(&path, original).unwrap();
        let store = SettingsStore::new(path.clone());

        let error = store.load().unwrap_err();

        assert!(error.to_string().contains("根节点必须是 JSON object"));
        assert_eq!(std::fs::read(path).unwrap(), original);
    }

    #[test]
    fn settings_store_load_invalid_known_field_returns_error_and_preserves_file() {
        let dir = temp_dir();
        let path = dir.join("settings.json");
        let original = br#"{"providerSyncEnabled":"yes","customField":"keep"}"#;
        std::fs::write(&path, original).unwrap();
        let store = SettingsStore::new(path.clone());

        let error = store.load().unwrap_err();

        assert!(error.to_string().contains("不兼容的字段或字段类型"));
        assert_eq!(std::fs::read(path).unwrap(), original);
    }
    ...
    #[test]
    fn settings_store_save_refuses_to_overwrite_corrupt_or_non_object_file() {
        let dir = temp_dir();
        for (index, original) in [
            "{bad json",
            r#"[{"providerSyncEnabled":true}]"#,
            r#"{"providerSyncEnabled":"yes","customField":"keep"}"#,
        ]
        .iter()
        .enumerate()
        {
            let path = dir.join(format!("settings-{index}.json"));
            std::fs::write(&path, original).unwrap();
            let store = SettingsStore::new(path.clone());

            let error = store.save(&BackendSettings::default()).unwrap_err();

            assert!(error.to_string().contains("原文件已保留"));
            assert_eq!(std::fs::read(&path).unwrap(), original.as_bytes());
        }
    }

    #[test]
    fn settings_store_update_refuses_to_overwrite_corrupt_or_non_object_file() {
        let dir = temp_dir();
        for (index, original) in [
            "{bad json",
            r#"[{"providerSyncEnabled":true}]"#,
    ...
    fn atomic_write_replaces_an_existing_file() {
        let dir = temp_dir();
        let path = dir.join("atomic-existing.json");
