...
#[tokio::test]
async fn settings_get_exposes_only_provider_ids_backed_by_config_tables() {
    for (relay_mode, official_mix_api_key) in [
        (RelayMode::PureApi, false),
        (RelayMode::MixedApi, false),
        (RelayMode::Official, true),
    ] {
        for provider in ["mirrorplus", "custom", "ccswitch"] {
            let settings = BackendSettings {
                active_relay_id: "friendly-profile-name".to_string(),
                relay_profiles: vec![RelayProfile {
                    id: "friendly-profile-name".to_string(),
                    relay_mode,
            ...
            assert_eq!(result["activeRelayCodexProvider"], json!(provider));
        }
    }
}

#[tokio::test]
async fn settings_get_does_not_invent_provider_for_missing_or_invalid_tables() {
    ...
    for config_contents in [
        ...
        let pure_api_ctx = BridgeContext::new(
            Arc::new(FakeSettings::with_settings_and_codex_home(
                pure_api,
                codex_home.clone(),
            )),
            Arc::new(FakeRuntime::default()),
        ...
        let result = handle_bridge_request(pure_api_ctx, "/settings/get", json!({})).await;
        assert_eq!(result["activeRelaySessionProvider"], json!(""));
        assert_eq!(result["activeRelayCodexProvider"], json!(""));
    }
...
#[tokio::test]
async fn settings_get_falls_back_to_valid_live_codex_provider() {
    let temp = tempfile::tempdir().unwrap();
    let codex_home = temp.path().join("codex-home");
    std::fs::create_dir_all(&codex_home).unwrap();
    std::fs::write(
    ...
    assert_eq!(result["activeRelaySessionProvider"], json!("custom"));
    assert_eq!(result["activeRelayCodexProvider"], json!("custom"));
}

#[tokio::test]
async fn settings_get_keeps_builtin_openai_for_official_login() {
    let official = BackendSettings::default();
    let official_ctx = BridgeContext::new(
        Arc::new(FakeSettings::with_settings(official)),
        Arc::new(FakeRuntime::default()),
        Arc::new(FakeData::default()),
    );
    let official_result = handle_bridge_request(official_ctx, "/settings/get", json!({})).await;
    assert_eq!(
        official_result["activeRelaySessionProvider"],
        json!("openai")
    );
    assert_eq!(official_result["activeRelayCodexProvider"], json!("openai"));
}

#[tokio::test]
async fn settings_get_fails_closed_when_active_profile_is_missing() {
    let settings = BackendSettings {
        active_relay_id: "deleted-profile".to_string(),
        ..BackendSettings::default()
    };
    let ctx = BridgeContext::new(
        Arc::new(FakeSettings::with_settings(settings)),
...
impl FakeSettings {
    ...
    fn with_settings_and_codex_home(settings: BackendSettings, codex_home: PathBuf) -> Self {
        Self {
            settings: Mutex::new(settings),
            codex_app_version: Mutex::new(String::new()),
...
impl BridgeSettingsService for FakeSettings {
    fn codex_home(&self) -> PathBuf {
        self.codex_home.clone().unwrap_or_else(|| {
            std::env::temp_dir().join(format!(
                "mirror-x-codex-bridge-tests-missing-home-{}",
                std::process::id()
            ))
        })
    }
