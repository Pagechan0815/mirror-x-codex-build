use codex_plus_core::relay_rotation::{
    ...
    fallback_relays_for_request, record_relay_request_failure, select_relay_for_probe,
    select_relay_for_request,
};
...
#[test]
fn model_routing_selects_only_the_key_group_that_contains_the_model() {
    let _guard = global_selector_test_lock();
    let mut settings = settings(AggregateRelayStrategy::Failover);
    settings.relay_profiles[0].model_list = "gpt-5.4\ngrok-4".to_string();
    settings.relay_profiles[1].model_list = "claude-opus-4-6".to_string();
    settings.relay_profiles[2].model_list = "gemini-3.1-pro".to_string();
    ...
    .unwrap();
    let fallbacks =
        fallback_relays_for_request(&settings, &selected.id, Some("claude-opus-4-6")).unwrap();
...
#[test]
fn model_routing_rejects_models_without_an_assigned_key() {
    let _guard = global_selector_test_lock();
    let mut settings = settings(AggregateRelayStrategy::Failover);
    settings.relay_profiles[0].model_list = "gpt-5.4\ngrok-4".to_string();
    settings.relay_profiles[1].model_list = "claude-opus-4-6".to_string();
    ...
    let error = select_relay_for_request(
        &settings,
        RotationContext {
    ...
    assert!(matches!(error, SelectionError::ModelNotRouted { .. }));
}
