use codex_plus_core::protocol_proxy::{
    ...
    open_models_proxy_request, open_responses_proxy_request, open_responses_proxy_request_at_path,
...
#[test]
fn responses_request_maps_kimi_coding_reasoning_effort() {
    ...
    let coding = responses_to_chat_completions(json!({
        "model": "kimi-for-coding",
        "reasoning": { "effort": "xhigh" },
        "input": "hi"
    }))
    .unwrap();
    assert_eq!(coding["reasoning_effort"], "max");

    let disabled = responses_to_chat_completions(json!({
        "model": "k3-256k",
        "reasoning": { "effort": "none" },
        "input": "hi"
    }))
...
#[test]
fn audio_transcriptions_proxy_path_matches_supported_prefixes() {
    assert!(is_audio_transcriptions_proxy_path("/audio/transcriptions"));
    assert!(is_audio_transcriptions_proxy_path(
        "/v1/audio/transcriptions"
    ));
    assert!(is_audio_transcriptions_proxy_path(
        "/codex/v1/audio/transcriptions?language=zh"
    ));
    assert!(!is_audio_transcriptions_proxy_path("/v1/responses"));
}
...
async fn responses_proxy_normalizes_legacy_custom_tool_item_ids_only() {
    ...
    let request = json!({
        "model": "gpt-5-mini",
        "input": [
            { "type": "custom_tool_call", "id": "fc_legacy", "call_id": "call_keep", "name": "exec", "input": "pwd" },
    ...
    assert!(raw.contains("\"id\":\"ctc_legacy\""));
    assert!(raw.contains("\"call_id\":\"call_keep\""));
...
#[tokio::test]
async fn responses_compact_proxy_preserves_compact_upstream_path() {
    let _lock = settings_path_test_lock().lock().unwrap();
    let temp = tempfile::tempdir().unwrap();
    let _guard = SettingsPathGuard::set(temp.path().join("settings.json"));
    let server = spawn_chat_server();
    write_responses_relay_settings(temp.path(), &server.base_url);

    let upstream = open_responses_proxy_request_at_path(
        r#"{"model":"claude-sonnet-4-6","input":"compact this","stream":false}"#,
        None,
        "/v1/responses/compact",
    )
    .await
...
#[tokio::test]
async fn audio_transcriptions_proxy_preserves_multipart_binary_body() {
    let _lock = settings_path_test_lock().lock().unwrap();
    let temp = tempfile::tempdir().unwrap();
    let _guard = SettingsPathGuard::set(temp.path().join("settings.json"));
    let listener = tokio::net::TcpListener::bind(("127.0.0.1", 0))
    ...
    .into_bytes();
    body.extend_from_slice(&[0x00, 0x80, 0xff, b'A', b'B']);
    ...
    assert!(
        headers
            .to_ascii_lowercase()
            .contains("content-type: multipart/form-data; boundary=codex-binary-boundary")
    );
    assert_eq!(&request[header_end + 4..], body);
}

#[tokio::test]
async fn proxy_rejects_corrupt_settings_without_falling_back_to_default_provider() {
    let _lock = settings_path_test_lock().lock().unwrap();
    let temp = tempfile::tempdir().unwrap();
    ...
    .await
    {
        Ok(_) => panic!("corrupt settings must block proxy routing"),
        Err(error) => error,
    };

    assert!(error.to_string().contains("读取供应商设置失败"));
    assert!(error.to_string().contains("未使用默认 Provider"));
...
fn write_responses_relay_settings(settings_dir: &Path, base_url: &str) {
    let settings = json!({
        "relayProfiles": [{
            "id": "responses",
            "name": "Responses",
