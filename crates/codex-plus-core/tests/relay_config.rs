...
"#,
    ...
    assert!(config.contains("[marketplaces.mirror-x-curated]"));
...
#[test]
fn relay_status_distinguishes_missing_files_from_unreadable_state() {
    let temp = tempfile::tempdir().unwrap();

    let missing = relay_config_status_from_home(temp.path());
    assert!(!missing.configured);
    assert!(!missing.state_unreadable);
    assert_eq!(missing.state_error, None);

    std::fs::create_dir(temp.path().join("config.toml")).unwrap();
    let unreadable = relay_config_status_from_home(temp.path());
    assert!(!unreadable.configured);
    assert!(unreadable.state_unreadable);
    assert!(
        unreadable
            .state_error
            .as_deref()
            .is_some_and(|message| message.contains("config.toml"))
    );
}

#[test]
fn relay_status_reports_invalid_config_and_auth_without_exposing_secrets() {
    let temp = tempfile::tempdir().unwrap();
    std::fs::write(
        temp.path().join("config.toml"),
        "model_provider = [\"secret-config-value\"]\n",
    )
    .unwrap();

    let invalid_config = relay_config_status_from_home(temp.path());
    assert!(invalid_config.state_unreadable);
    let config_error = invalid_config.state_error.unwrap();
    assert!(config_error.contains("model_provider"));
    assert!(!config_error.contains("secret-config-value"));
    ...
    .unwrap();
    let malformed_config = relay_config_status_from_home(temp.path());
    assert!(malformed_config.state_unreadable);
    let malformed_error = malformed_config.state_error.unwrap();
    assert!(malformed_error.contains("config.toml"));
    assert!(!malformed_error.contains("secret-config-value"));
    ...
    std::fs::write(
        temp.path().join("auth.json"),
        r#"{"OPENAI_API_KEY":"secret-auth-value""#,
    )
    .unwrap();

    let invalid_auth = relay_config_status_from_home(temp.path());
    assert!(invalid_auth.state_unreadable);
    let auth_error = invalid_auth.state_error.unwrap();
    assert!(auth_error.contains("auth.json"));
    assert!(!auth_error.contains("secret-auth-value"));
}
...
#[test]
fn set_codex_goals_feature_rejects_invalid_existing_toml_without_overwriting() {
    let temp = tempfile::tempdir().unwrap();
...
#[test]
fn set_codex_goals_feature_rejects_unreadable_config_path_without_replacing_it() {
    let temp = tempfile::tempdir().unwrap();
...
#[test]
fn clear_relay_config_rejects_invalid_existing_toml_without_overwriting() {
    let temp = tempfile::tempdir().unwrap();
...
"#;
    ...
    let error = clear_relay_config_to_home(temp.path()).unwrap_err();
    assert!(error.to_string().contains("TOML"));
...
#[test]
fn clear_relay_config_rejects_unreadable_config_path_without_replacing_it() {
    let temp = tempfile::tempdir().unwrap();
    let config_path = temp.path().join("config.toml");
    std::fs::create_dir(&config_path).unwrap();

    assert!(clear_relay_config_to_home(temp.path()).is_err());
    assert!(config_path.is_dir());
}
...
#[test]
fn backfill_pure_api_profile_preserves_stored_key_over_live_auth() {
    let temp = tempfile::tempdir().unwrap();
    std::fs::write(
        temp.path().join("config.toml"),
        r#"model_provider = "custom"
...
"#
    ...
    let live_doc = live_config.parse::<toml_edit::DocumentMut>().unwrap();
    assert!(live_doc.get("profile").is_none());
    assert!(live_config.contains("[profiles.work]"));
...
"#
    ...
    assert!(
        parsed["models"]
            ...
            .unwrap()
            .iter()
            .all(|model| model["use_responses_lite"] == false)
    );
}
...
"#
    ...
    let catalog: serde_json::Value = serde_json::from_str(
        &std::fs::read_to_string(temp.path().join("model-catalogs/relay-gpt56.json")).unwrap(),
    )
    .unwrap();
...
#[test]
fn apply_relay_profile_copies_external_lite_catalog_for_custom_responses() {
    let temp = tempfile::tempdir().unwrap();
    let external_dir = temp.path().join("external");
    std::fs::create_dir_all(&external_dir).unwrap();
    let external_catalog = external_dir.join("gpt56.json");
    let external_contents = r#"{
  "models": [
    {
      "slug": "gpt-5.6-sol",
    ...
    std::fs::write(&external_catalog, external_contents).unwrap();
...
"#
    ...
    assert_eq!(
        std::fs::read_to_string(&external_catalog).unwrap(),
        external_contents
    );
}

#[test]
fn apply_custom_chat_profile_preserves_external_lite_catalog() {
    let temp = tempfile::tempdir().unwrap();
    let external_dir = temp.path().join("external");
    std::fs::create_dir_all(&external_dir).unwrap();
    let external_catalog = external_dir.join("gpt56-chat.json");
    let external_contents = r#"{"models":[{"slug":"gpt-5.6-sol","use_responses_lite":true}]}"#;
    std::fs::write(&external_catalog, external_contents).unwrap();
    let external_absolute = external_catalog.to_string_lossy();
    let profile = RelayProfile {
        id: "relay-gpt56-chat-external".to_string(),
        model: "gpt-5.6-sol".to_string(),
        relay_mode: RelayMode::PureApi,
        config_contents: format!(
            r#"model = "gpt-5.6-sol"
...
#[test]
fn apply_relay_profile_copies_live_external_lite_catalog_for_custom_responses() {
    let temp = tempfile::tempdir().unwrap();
    let external_dir = temp.path().join("external");
    std::fs::create_dir_all(&external_dir).unwrap();
    let external_catalog = external_dir.join("live-gpt56.json");
    let external_contents = r#"{"models":[{"slug":"gpt-5.6-sol","supports_search_tool":true,"use_responses_lite":true}]}"#;
    std::fs::write(&external_catalog, external_contents).unwrap();
    std::fs::write(
        temp.path().join("config.toml"),
        format!(
            "model_catalog_json = '{}'\n",
            external_catalog.to_string_lossy()
        ),
    )
    .unwrap();
    let profile = RelayProfile {
        id: "relay-live-gpt56".to_string(),
        model: "gpt-5.6-sol".to_string(),
        relay_mode: RelayMode::PureApi,
        config_contents: r#"model = "gpt-5.6-sol"
model_provider = "custom"
...
"#
    ...
    let config = std::fs::read_to_string(temp.path().join("config.toml")).unwrap();
    assert!(config.contains(r#"model_catalog_json = "model-catalogs/relay-live-gpt56.json""#));
    let copied: serde_json::Value = serde_json::from_str(
        &std::fs::read_to_string(
            temp.path()
                .join("model-catalogs")
                .join("relay-live-gpt56.json"),
        )
        .unwrap(),
    )
    .unwrap();
...
#[test]
fn apply_relay_profile_removes_cc_switch_catalog_without_custom_model_windows() {
    let temp = tempfile::tempdir().unwrap();
    let profile = RelayProfile {
        id: "relay-a".to_string(),
        model: "qwen3-coder".to_string(),
