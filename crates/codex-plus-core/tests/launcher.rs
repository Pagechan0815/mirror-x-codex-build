...
use codex_plus_core::app_paths::{
    ...
    find_latest_codex_app_dir_from_roots, find_macos_codex_app, is_codex_app_launchable,
    latest_appx_install_location_from_output, normalize_codex_app_path, packaged_app_user_model_id,
    resolve_codex_app_dir_with_saved, standalone_codex_app_candidates_from,
...
#[test]
fn app_paths_prefers_stable_package_over_newer_beta_and_legacy_builds() {
    let temp = tempfile::tempdir().unwrap();
    let stable = temp
        .path()
        .join("OpenAI.Codex_26.500.0.0_x64__2p2nqsd0c76g0/app");
    let beta = temp
        .path()
        .join("OpenAI.CodexBeta_99.0.0.0_x64__2p2nqsd0c76g0/app");
    let legacy = temp
        .path()
        .join("OpenAI.ChatGPT-Desktop_98.0.0.0_x64__2p2nqsd0c76g0/app");
    for path in [&stable, &beta, &legacy] {
        std::fs::create_dir_all(path).unwrap();
    }

    assert_eq!(
        find_latest_codex_app_dir(temp.path()).as_deref(),
        Some(stable.as_path())
    );
}

#[test]
fn app_paths_uses_legacy_package_before_beta_when_stable_is_absent() {
    let temp = tempfile::tempdir().unwrap();
    let beta = temp
        .path()
        .join("OpenAI.CodexBeta_99.0.0.0_x64__2p2nqsd0c76g0/app");
    let legacy = temp
        .path()
        .join("OpenAI.ChatGPT-Desktop_26.500.0.0_x64__2p2nqsd0c76g0/app");
    for path in [&beta, &legacy] {
        std::fs::create_dir_all(path).unwrap();
    }
...
#[test]
fn app_paths_standalone_candidates_cover_codex_and_chatgpt_installers() {
    let local = PathBuf::from(r"C:\Users\me\AppData\Local");
    let candidates = standalone_codex_app_candidates_from(&local);

    assert_eq!(candidates[0], local.join("OpenAI/Codex/bin"));
    assert!(candidates.contains(&local.join("Programs/OpenAI/Codex")));
    assert!(candidates.contains(&local.join("OpenAI/ChatGPT/bin")));
    assert!(candidates.contains(&local.join("Programs/OpenAI/ChatGPT")));
    assert!(candidates.contains(&local.join("Programs/ChatGPT")));
}

#[test]
fn app_paths_builds_chatgpt_fallback_for_store_package_without_readable_payload() {
...
#[test]
fn app_paths_launchable_accepts_real_executable_and_rejects_fake_store_path() {
    let temp = tempfile::tempdir().unwrap();
    let standalone = temp.path().join("Codex");
    std::fs::create_dir_all(&standalone).unwrap();
    std::fs::write(standalone.join("ChatGPT.exe"), "test").unwrap();
    assert!(is_codex_app_launchable(&standalone));

    let fake_store_app =
        PathBuf::from(r"C:\Program Files\WindowsApps\OpenAI.Codex_0.0.0.0_x64__notregistered\app");
    assert!(!is_codex_app_launchable(&fake_store_app));
}

#[cfg(windows)]
#[test]
fn app_paths_explicit_unregistered_store_path_is_rejected() {
    let fake_store_app =
        PathBuf::from(r"C:\Program Files\WindowsApps\OpenAI.Codex_0.0.0.0_x64__notregistered\app");

    assert_eq!(
        resolve_codex_app_dir_with_saved(Some(&fake_store_app), None),
        None
    );
}

#[test]
fn app_paths_extracts_standalone_version_directory_and_file() {
    let temp = tempfile::tempdir().unwrap();
    let versioned = temp.path().join("26.814.5517.0");
    std::fs::create_dir_all(&versioned).unwrap();
    assert_eq!(
        codex_app_version(&versioned).as_deref(),
        Some("26.814.5517.0")
    );

    let version_file_dir = temp.path().join("Codex");
    std::fs::create_dir_all(&version_file_dir).unwrap();
    std::fs::write(version_file_dir.join("version"), "26.900.1\n").unwrap();
    assert_eq!(
        codex_app_version(&version_file_dir).as_deref(),
        Some("26.900.1")
    );
}
...
#[test]
fn app_paths_find_macos_codex_app_supports_chatgpt_bundle() {
    let temp = tempfile::tempdir().unwrap();
...
#[test]
fn app_paths_build_macos_bundle_executable_falls_back_to_codex() {
...
#[test]
fn app_paths_rejects_existing_directories_and_files_without_codex() {
    let temp = tempfile::tempdir().unwrap();
    let invalid_dir = temp.path().join("stale-saved-path");
    let invalid_file = temp.path().join("not-codex.exe");
    std::fs::create_dir_all(&invalid_dir).unwrap();
    std::fs::write(&invalid_file, "not an executable").unwrap();

    assert_eq!(normalize_codex_app_path(&invalid_dir), None);
    assert_eq!(normalize_codex_app_path(&invalid_file), None);
}

#[test]
fn app_paths_rejects_standalone_lowercase_codex_cli() {
    let temp = tempfile::tempdir().unwrap();
    let cli = temp.path().join("codex.exe");
    std::fs::write(&cli, b"cli").unwrap();

    assert!(normalize_codex_app_path(&cli).is_none());
    assert!(!is_codex_app_launchable(temp.path()));
}
...
fn app_paths_saved_path_is_used_when_no_explicit_path_is_provided() {
    ...
    std::fs::write(app.join("Contents/MacOS/Codex"), "test").unwrap();
...
#[test]
fn app_paths_explicit_stale_directory_is_not_a_launch_target() {
    let temp = tempfile::tempdir().unwrap();
    let stale = temp.path().join("Codex.app");
    std::fs::create_dir_all(&stale).unwrap();

    assert_eq!(resolve_codex_app_dir_with_saved(Some(&stale), None), None);
}
...
#[test]
fn launcher_applies_mirrorplus_window_icon_after_packaged_activation() {
    let source = include_str!("../src/launcher.rs");

    assert!(source.contains("apply_mirrorplus_window_icon_after_launch(process_id);"));
    assert!(source.contains("windows_apply_mirrorplus_icon_to_process_window"));
}

/// The mobile control runtime is owned by the manager application, never by the
/// launcher. Keeping it out of `launcher.rs` guarantees that enabling phone
/// control cannot perturb the Codex injection path.
#[test]
fn launcher_stays_free_of_mobile_control_runtime() {
...
/// Mobile control must ship disabled and must default to the TLS relay, because
/// iOS refuses plain `ws://` from an https page.
#[test]
fn mobile_control_settings_are_opt_in_and_tls_by_default() {
    let settings = codex_plus_core::settings::BackendSettings::default();

    assert!(!settings.mobile_control_enabled);
    assert!(settings.mobile_control_relay_url.starts_with("wss://"));
}
...
fn launcher_plugin_marketplace_unlock_repairs_role_specific_plugins() {
    ...
    assert!(launcher_source.contains("ensure_required_plugin_marketplaces(&home)"));
...
#[tokio::test]
async fn launch_lifecycle_does_not_rewrite_history_for_official_profile() {
    let temp = tempfile::tempdir().unwrap();
    ...
    assert!(!events.contains(&"managed-provider".to_string()));
...
#[tokio::test]
async fn launch_settings_failure_records_terminal_status_before_starting_codex() {
    let temp = tempfile::tempdir().unwrap();
    let app_dir = temp.path().join("Codex.app");
    std::fs::create_dir_all(&app_dir).unwrap();
    let status_store = StatusStore::new(temp.path().join("latest-status.json"));
    let events = Arc::new(Mutex::new(Vec::<String>::new()));
    let hooks = FakeHooks::new(events.clone())
        .with_settings_error("settings.json 不是有效 JSON；原文件已保留，未使用默认设置覆盖");
    ...
    .await
    .expect_err("invalid settings must stop launch");

    assert!(error.to_string().contains("已停止启动"));
    assert_eq!(
        *events.lock().unwrap(),
        vec![
            "select-debug:9229",
    ...
    assert!(status.message.contains("原文件已保留"));
    assert_eq!(
        status.codex_app.as_deref(),
        Some(app_dir.to_string_lossy().as_ref())
    );
}
...
#[tokio::test]
async fn launch_lifecycle_checks_managed_provider_before_and_after_other_config_writers() {
    let temp = tempfile::tempdir().unwrap();
    let app_dir = temp.path().join("Codex.app");
    std::fs::create_dir_all(&app_dir).unwrap();
    let status_store = StatusStore::new(temp.path().join("latest-status.json"));
    ...
    let hooks = FakeHooks::new(events.clone()).with_settings(BackendSettings {
        relay_profiles_enabled: true,
        active_relay_id: codex_plus_core::mirror_access::MIRROR_PROVIDER_ID.to_string(),
        computer_use_guard_enabled: true,
        relay_profiles: vec![RelayProfile {
            id: codex_plus_core::mirror_access::MIRROR_PROVIDER_ID.to_string(),
...
"#
    ...
    let events = events.lock().unwrap().clone();
    let provider_checks = events
        .iter()
        .enumerate()
        .filter_map(|(index, event)| (event == "managed-provider").then_some(index))
        .collect::<Vec<_>>();
    assert_eq!(provider_checks.len(), 2);
    let config_writer_index = events
        .iter()
        .position(|event| event == "computer-use-guard")
        .expect("test config writer must run");
    let launch_index = events
        .iter()
        .position(|event| event == "launch:9229")
        .expect("Codex must be launched");
    assert!(provider_checks[0] < config_writer_index);
    assert!(config_writer_index < provider_checks[1]);
    assert!(provider_checks[1] < launch_index);
}
...
#[tokio::test]
async fn launch_lifecycle_does_not_rewrite_generic_relay_profiles() {
    let temp = tempfile::tempdir().unwrap();
    let app_dir = temp.path().join("Codex.app");
    std::fs::create_dir_all(&app_dir).unwrap();
    let status_store = StatusStore::new(temp.path().join("latest-status.json"));
...
#[tokio::test]
async fn launch_stops_before_helper_and_codex_when_managed_provider_repair_fails() {
    let temp = tempfile::tempdir().unwrap();
    ...
    let hooks = FakeHooks::new(events.clone())
        ...
        .with_managed_provider_error("managed provider table is missing");
    ...
    .await
    .expect_err("managed provider repair failure must block launch");

    assert!(format!("{error:#}").contains("managed provider table is missing"));
    assert_eq!(
...
#[tokio::test]
async fn launch_stops_before_codex_when_required_plugin_marketplace_config_fails() {
    let temp = tempfile::tempdir().unwrap();
    let events = Arc::new(Mutex::new(Vec::new()));
    let hooks = FakeHooks::new(events.clone())
    ...
    .await
    .expect_err("required plugin configuration must block launch");

    assert!(error.to_string().contains("config.toml TOML parse failed"));
    assert_eq!(
        events.lock().unwrap().as_slice(),
        [
            "select-debug:9229",
...
struct FakeHooks {
    ...
    managed_provider_error: Option<String>,
...
impl FakeHooks {
    ...
    fn with_settings_error(mut self, message: &str) -> Self {
        self.settings_error = Some(message.to_string());
        self
    }
    ...
    fn with_managed_provider_error(mut self, message: &str) -> Self {
        self.managed_provider_error = Some(message.to_string());
        self
    }
...
impl LaunchHooks for FakeHooks {
    ...
    async fn ensure_managed_provider_ready(
        ...
        if settings.relay_profiles_enabled
            && settings.active_relay_id == codex_plus_core::mirror_access::MIRROR_PROVIDER_ID
        {
            self.event("managed-provider");
            if let Some(message) = &self.managed_provider_error {
                anyhow::bail!(message.clone());
            }
        }
        Ok(())
