...
#[test]
fn build_catalog_json_preserves_official_template_responses_lite_behavior() {
...
#[test]
fn model_ui_metadata_exposes_gpt56_reasoning_and_fast_capabilities() {
    let metadata = model_ui_metadata("gpt-5.6-sol").expect("Sol metadata should exist");

    assert_eq!(metadata["defaultReasoningEffort"], "low");
    assert_eq!(
    ...
    assert_eq!(
        metadata["supportedReasoningEfforts"][5]["reasoningEffort"],
        "ultra"
    );
}
